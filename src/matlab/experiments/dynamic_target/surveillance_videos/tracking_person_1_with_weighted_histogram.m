...
clc; clear;
video = "busy_constant_lighting.mp4";
...
%% Tune initial histogram - strip shirt large region
...
% Parameters
center_x_coordinate = 410;
center_y_coordinate = 245;
rect_width_initial = 20;
rect_height_initial = 60; 

% Show tuning
imshow(reference_frame);
hold on;
xLeft = round(center_x_coordinate - rect_width_initial/2);
yBottom = round(center_y_coordinate - rect_height_initial/2);
rectangle('Position',[xLeft,yBottom,rect_width_initial,rect_height_initial],'EdgeColor','g','LineWidth',1);

%% Tune initial histogram - striped shirt only
reference_frame= frames(:,:,:,1);
reference_frame_hsv = hsv_frames(:,:,:,1);
...
% Parameters
center_x_coordinate = 409;
center_y_coordinate = 235;
rect_width_initial = 20;
rect_height_initial = 20; 
...
%% Tune initial histogram - red shirt
reference_frame= frames(:,:,:,1);
reference_frame_hsv = hsv_frames(:,:,:,1);
...
% Parameters
center_x_coordinate = 406;
center_y_coordinate = 312;
rect_width_initial = 20;
rect_height_initial = 20; 
...
%% Show mask
[logical_image, out_of_image] = get_rectangle_mask_from_sample([center_y_coordinate;center_x_coordinate],image_height,image_width,rect_height_initial,rect_width_initial);
imshow(bsxfun(@times, reference_frame, cast(logical_image, 'like', reference_frame)));

%% Get reference histogram
a = sqrt((rect_width_initial/2)^2 + (rect_height_initial/2)^2);
tic;
...
R = diag([15 15 20 20])*scale;                                  % process noise 
...
rect_width = rect_width_initial;
rect_height = rect_height_initial; 
...
for i = 1:numFrames
    ...
    for hist_index = 1:M
        ...
        a = sqrt((rect_width/2)^2 + (rect_height/2)^2);        
